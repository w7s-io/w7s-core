...
`routing.defaultDomain` controls whether a deployment is served from the default `*.w7s.cloud` URL. It defaults to `true`. Set it to `false` when the app should only be reachable through a custom domain declared in `CNAME`:
