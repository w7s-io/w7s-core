...
- `src/deploy/aiBindings.ts`
  - Adds the W7S AI service binding and per-deployment bearer token to native Workers.
- `src/api/ai.ts`
  - Resolves callers from W7S AI bearer tokens, applies app suspension and `ai.run` limits, and calls the core Workers AI binding.
